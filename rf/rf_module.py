...
class RfModule(object):
	...
	def generateModel(self):
		self.regressor = RandomForestRegressor(n_estimators=20, random_state=0)
		self.regressor.fit(self.data_set_samples, self.data_set_labels)

	#funcao utilizada para retornar o resultado da classificacao em 1 ou 0
	def predictClasses(self):
		predictions = self.regressor.predict(self.test_data_set_samples)
		return predictions
